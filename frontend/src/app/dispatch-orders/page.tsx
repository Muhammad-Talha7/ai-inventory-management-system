...
interface DispatchOrder {
  ...
  approved_by?: number;
  rejected_by?: number;
  dispatching_notes?: string;
  destination?: string;
  items: DispatchOrderItem[];
}

function DispatchOrdersPageContent() {
  const { user } = useAuth();
  ...
  const [selectedPoForDispatch, setSelectedPoForDispatch] = useState<DispatchOrder | null>(null);
  const [receiveQuantities, setDispatchQuantities] = useState<Record<number, number>>({});
  const [receiveNotes, setDispatchNotes] = useState('');
  const [receiving, setReceiving] = useState(false);

  // Reject Modal State
  const [showRejectModal, setShowRejectModal] = useState(false);
  const [selectedPoForReject, setSelectedPoForReject] = useState<DispatchOrder | null>(null);
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await apiFetch(`/dispatch-orders/?page=${page}`);
      if (res.success) {
        setOrders(res.data);
  ...
  const handleCompleteOrder = async (orderId: number) => {
    try {
      await apiFetch(`/dispatch-orders/${orderId}/approve`, {
        method: 'POST',
      });
      fetchOrders();
    } catch (err: any) {
  ...
  const handleOpenDispatchModal = (order: DispatchOrder) => {
    setSelectedPoForDispatch(order);
  ...
  const handleSubmitDispatch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch(`/dispatch-orders/${selectedPoForDispatch.dispatch_id}/dispatch`, {
        method: 'POST',
        body: JSON.stringify({
          items: payloadItems,
          dispatching_notes: receiveNotes
        })
      });
  ...
  const handleOpenRejectModal = (order: DispatchOrder) => {
  ...
  const handleSubmitReject = async (e: React.FormEvent) => {
    ...
    try {
      setRejecting(true);
      const res = await apiFetch(`/dispatch-orders/${selectedPoForReject.dispatch_id}/reject`, {
        method: 'POST',
        body: JSON.stringify({
          reason: rejectReason
        })
      ...
      if (res.success) {
        ...
        setOrders(orders.map(o => o.dispatch_id === selectedPoForReject.dispatch_id ? { ...o, status: 'Rejected' } : o));
      }
    } catch (err: any) {
  ...
  const handleOpenManualModal = () => {
    ...
    if (products.length > 0) {
      setManualItems([{product_id: products[0].product_id, quantity: 1, tempId: nextTempId}]);
  ...
  const handleOpenEditModal = (order: DispatchOrder) => {
    setEditingOrder(order);
    setDestinationText(order.destination || '');
  ...
  const handleUpdateItems = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch(`/dispatch-orders/${editingOrder.dispatch_id}/items`, {
        method: 'PATCH',
  ...
  const handleDestinationChange = (suppId: string) => {
    setDestinationText(suppId);
    setManualItems([]);
    const suppProducts = products.filter(p => p.destination === suppId);
  ...
  const handleCreateManualOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch('/dispatch-orders/manual', {
        method: 'POST',
        body: JSON.stringify({
    ...
    <div className="max-w-[1280px] mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-black tracking-tight text-slate-900">Dispatch Orders</h1>
        ...
        <div className="flex gap-3">
          ...
          {user?.role === 'staff' && (
            <button
              onClick={() => window.location.href = '/scanner?mode=SO'}
              className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 text-slate-700 font-bold rounded-2xl shadow-sm hover:bg-slate-50 transition uppercase tracking-widest text-xs"
            >
            ...
            <table className="w-full">
              ...
              <tbody>
                {loading ? (
                  ...
                  orders.map((o) => (
                    <React.Fragment key={`po-${o.dispatch_id}`}>
                      <tr className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                        <td className="px-4 py-5 text-center">
                          <button onClick={() => toggleExpand(o.dispatch_id)} className="p-1 text-slate-400 hover:text-slate-600 rounded">
                            {expandedOrders.has(o.dispatch_id) ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                          </button>
                        </td>
                        ...
                        <td className="px-6 py-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            {o.status === 'Pending Approval' && user?.role === 'manager' && (
                              <button
                                onClick={() => handleCompleteOrder(o.dispatch_id)}
                            ...
                            {(o.status === 'Scheduled' || o.status === 'Pending Approval') && user?.role === 'manager' && (
                              <>
                                ...
                                <button
                                  onClick={() => handleCancelOrder(o.dispatch_id)}
                      ...
                      {expandedOrders.has(o.dispatch_id) && (
                        <tr key={`items-${o.dispatch_id}`} className="bg-slate-50/50">
                          <td></td>
                              ...
                              <table className="w-full">
                                <thead>
                                  <tr className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                    ...
                                    <th className="text-right px-4 py-2">Dispatchd Qty</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {o.items.map(item => (
                                    <tr key={item.id} className="border-b border-slate-50 last:border-0">
                                      ...
                                      <td className="px-4 py-3 text-sm font-bold text-indigo-600 text-right">
                                        {item.dispatched_quantity !== null && item.dispatched_quantity !== undefined ? item.dispatched_quantity : '-'}
                                      </td>
                                    </tr>
                                  ))}
                                </tbody>
          ...
          <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-4 grow">
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-black text-slate-500 uppercase tracking-widest mb-2">Destination</label>
                  <input type="text" value={destinationText} onChange={(e) => setDestinationText(e.target.value)} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" placeholder="e.g., Store #42, Warehouse B, Client XYZ" />
                </div>
      ...
      {showDispatchModal && selectedPoForDispatch && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <h2 className="text-xl font-black text-slate-900">Dispatch Order #{selectedPoForDispatch.dispatch_id}</h2>
              <button onClick={() => setShowDispatchModal(false)} className="text-slate-400 hover:text-slate-600">
                <XCircle size={24} />
              </button>
            </div>
            <form onSubmit={handleSubmitDispatch} className="p-6 overflow-y-auto space-y-6 grow">
              
              <div className="space-y-3">
                <div className="flex text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">
                  <div className="flex-1">Product</div>
                  <div className="w-24 text-center">Ordered</div>
                  <div className="w-24 text-center">Dispatchd</div>
                </div>
                
                {selectedPoForDispatch.items.map(item => (
                  <div key={item.id} className="flex gap-3 items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                    ...
                    <div className="w-24">
                      <input
                        ...
                        onChange={e => setDispatchQuantities({...receiveQuantities, [item.id]: parseInt(e.target.value) || 0})}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-xl font-black text-slate-900">Reject Order #{selectedPoForReject.dispatch_id}</h2>
      ...
      {showEditModal && editingOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <h2 className="text-xl font-black text-slate-900">Edit Items for Order #{editingOrder.dispatch_id}</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl">
            ...
            <div className="p-6 space-y-4">
              <p className="text-slate-600 text-sm font-medium">Are you sure you want to cancel Dispatch Order #{poToCancel}?</p>
              <div className="pt-4 flex gap-3">
                <button
                  type="button"
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      await apiFetch(`/dispatch-orders/${orderId}/status?status=Cancelled`, {
                        method: 'PATCH',
                      });
                      setOrders(orders.map(o => o.dispatch_id === orderId ? { ...o, status: 'Cancelled' } : o));
                    } catch (err: any) {
  ...
  return (
    <Suspense fallback={
      <div className="h-[80vh] flex flex-col items-center justify-center">
        ...
        <p className="text-slate-500 font-medium">Loading Dispatch Orders...</p>
      </div>
    }>
      <DispatchOrdersPageContent />
    </Suspense>
  );
}
